...
public class SearchableDropDown : MonoBehaviour
{
    ...
    {
        // Add a delay before checking input
        yield return new WaitForSeconds(1);

        if (!avlOptions.Contains(arg))
        {
            // If the input is not valid, reset the input field
            inputField.text = string.Empty;
            // Display a message informing the user about invalid input (commented out for now)
        ...
        {
            // If the input is valid, you can perform other actions here
            Debug.Log("good job ");
        }
        // Invoke the event with the input text, whether it's valid or not
